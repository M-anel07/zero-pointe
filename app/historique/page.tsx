...
const OFFICIEL_CATEGORIES = ['Shopping', 'Restaurant', 'Beauté', 'Tech', 'Nourriture', 'Jeux']
...
function getCategoryBadgeStyle(category: string) {
    ...
    // Si c'est une catégorie officielle fixe, style gris neutre du screenshot
    if (OFFICIEL_CATEGORIES.includes(category)) {
        return 'bg-zinc-900/40 border border-zinc-800 text-zinc-300 font-extrabold'
    }

    // Si c'est une catégorie "Autre" custom -> couleur aléatoire mais stable
    let hash = 0
...
async function getDepenses(userEmail: string, categorie?: string, tri?: string) {
    const orderBy = tri === 'prix_asc' ? { prix: 'asc' as const } : tri === 'prix_desc' ? { prix: 'desc' as const } : { creerLe: 'desc' as const }
    ...
    return await prisma.depense.findMany({
        where: { user: { email: userEmail }, ...(categorie ? { category: categoryCondition } : {}) },
        orderBy,
        include: { votes: true },
    })
}
...
export default async function PageHistorique({ searchParams }: PageProps) {
    ...
    const categoryTotals = allDepenses.reduce<Record<string, number>>((acc, d) => {
        const displayCategory = OFFICIEL_CATEGORIES.includes(d.category) ? d.category : 'Autre'
    ...
    return (
        <div className="min-h-screen bg-[#080808] text-zinc-100 flex flex-col antialiased">
            ...
            <header className="border-b border-white/15 py-5 sticky top-0 z-10 bg-[#080808]/95 backdrop-blur-md w-full">
                <div className="w-full px-8 flex items-center justify-between">
                    <div className="flex items-center gap-5">
                        <Link href="/dashboard" className="flex items-center gap-2 text-zinc-300 hover:text-white transition-colors text-sm font-semibold">
                        ...
                        <h1 className="text-xl font-black tracking-tight text-[#FF4A7D] leading-none">Mon Historique</h1>
                    </div>
                    <div className="hidden sm:flex flex-col items-end gap-1">
                        <span className="text-[10px] uppercase tracking-[2.5px] text-zinc-300 font-bold">total dépensé</span>
            ...
            <main className="flex-1 w-full px-8 py-10 flex flex-col gap-8">
                ...
                {/* ── Bloc Top Catégories Strictement conforme à ton HTML ── */}
                {topCategories.length > 0 && (
                    <div>
                        <p className="text-[11px] uppercase tracking-[2.5px] text-[#A7E0E0] font-extrabold mb-4">Top catégories</p>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
                            {topCategories.map(([cat, catTotal], i) => {
                                const rank = ['#1', '#2', '#3'][i]
                                return (
                                    <div key={cat} className="relative rounded-2xl border px-5 py-5 flex flex-col gap-4 bg-zinc-900/40 border-zinc-800">
                                        <span className="absolute top-4 right-4 text-[10px] font-black text-zinc-300">{rank}</span>
                                        <div>
                                            <p className="text-[10px] uppercase tracking-[2px] text-[#FF4A7D] font-bold">{cat}</p>
                ...
                {/* Filtres */}
                <FiltresHistorique categories={FILTRES_CATEGORIES} categorieActive={categorie} triActif={tri} />
                ...
                {depenses.length === 0 ? (
                    ...
                    <div className="bg-[#0d0d0d] border border-white/15 rounded-2xl overflow-hidden shadow-xl w-full">
                        <div className="px-6 py-4 border-b border-white/10 bg-white/[0.02] flex items-center justify-between">
                            <p className="text-[11px] uppercase tracking-[2.5px] text-zinc-300 font-extrabold">dépenses</p>
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-white/10">
                                    {depenses.map((depense) => {
                                        ...
                                        // Applique le bon style de badge selon la nature de la catégorie
                                        const badgeClass = getCategoryBadgeStyle(depense.category)

                                        return (
                                            <tr key={depense.id} className={`transition-colors group ${isTopCraquage ? 'bg-gradient-to-r from-red-950/20 via-transparent to-transparent hover:bg-red-950/30' : 'hover:bg-white/[0.04]'}`}>
                                                <td className="py-4 px-6 text-sm font-bold text-white group-hover:text-[#FF4A7D] transition-colors max-w-xs truncate">{depense.titre}</td>
                                                <td className="py-4 px-6">
                                                    <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full ${badgeClass}`}>
                                                        {isTopCraquage ? 'TOP CRAQUAGE' : depense.category}
                                                    </span>
                                                </td>
                                                <td className="py-4 px-6 text-[11px] text-zinc-400 font-semibold">{formatDate(depense.creerLe)}</td>
                                                <td className="py-4 px-6">
                                                    <div className="flex items-center justify-center gap-4">
                                                        <span className="flex items-center gap-1 text-[11px] font-bold" style={{ color: '#A7E0E0' }}>
                                                            <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3H14z" /><path d="M7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3" /></svg>{validated}
                                                        </span>
                                                        <span className="flex items-center gap-1 text-[11px] font-bold text-[#FF4A7D]">
                                                            <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3H10z" /><path d="M17 2h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17" /></svg>{shameful}
                                                        </span>
                                                    </div>
                                                </td>
            ...
            <footer className="border-t border-white/10 py-6 mt-4 bg-black/40">
                <div className="w-full px-8 text-center">
                    <p className="text-zinc-400 text-xs font-medium">&copy; {new Date().getFullYear()} Zéro Pointé — aucun droit réservé.</p>
                </div>
            </footer>
        </div>
    )
